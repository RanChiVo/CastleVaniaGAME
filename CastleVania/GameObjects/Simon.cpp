...
constexpr float SIMON_JUMP_SPEED_Y = 0.45f;
...
void Simon::RenderWeapon(LPANIMATION animation, Viewport * viewport)
{
	if (startAtack)
	{
		whip->updatePostision(animation_set->find(currentAnimation)->second->getCurrentFrame(), currentAnimation, flip);
...
void Simon::Render(Viewport* viewport)
{
	...
	case SIMON_STATE_JUMPING:
		...
		else currentAnimation = SIMON_ANI_IDLE;
		break;
	default:
		currentAnimation = SIMON_ANI_IDLE;
		break;
